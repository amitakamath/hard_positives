...
def save_scores(scores, args):
    ...
    try:
        ...
        escaped_name = '-'.join(args.model_name.split('/'))
        si2t_file = os.path.join(args.output_dir, "scores", f"{args.dataset}--{escaped_name}--{args.seed}--si2t.pkl")
        st2i_file = os.path.join(args.output_dir, "scores", f"{args.dataset}--{escaped_name}--{args.seed}--st2i.pkl")

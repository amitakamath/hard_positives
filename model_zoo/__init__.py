...
def get_model(model_name, device, root_dir=CACHE_DIR):
    ...
    if "openai-clip" in model_name:
    ...
    elif 'svlc' in model_name or 'dac' in model_name:
        from TSVLC.src.open_clip import create_model_and_transforms
        from .clip_models import CLIPWrapper
        path = "replace/models/{}.pt".format(model_name)
        print("Getting model from {}...".format(path))
        model, _, image_preprocess = create_model_and_transforms('ViT-B-32', pretrained=path, device=device, precision="amp", lora=4)
        model = model.eval()
        clip_model = CLIPWrapper(model, device) 
        return clip_model, image_preprocess
    
    elif 'crepe' in model_name or 'coco' in model_name:
        import open_clip
        from .clip_models import CLIPWrapper
        path = "replace/models/{}.pt".format(model_name)
        print("Getting model from {}...".format(path))
        ...
        elif variant == 'ViT-g-14':
            pretrained = 'laion2b_s34b_b88k'
        elif variant == 'ViT-bigG-14':
            pretrained = 'laion2b_s39b_b160k'
        elif variant == 'roberta-ViT-B-32':
            pretrained = 'laion2b_s12b_b32k'
        elif variant == 'coca_ViT-B-32':
            pretrained = 'laion2b_s13b_b90k'
        elif variant == 'dc-ViT-B-32':
            variant = 'ViT-B-32'
            pretrained = 'datacomp_xl_s13b_b90k'
        elif variant == 'dc-ViT-B-16':
            variant = 'ViT-B-16'
            pretrained = 'datacomp_xl_s13b_b90k'
        elif variant == 'dc-ViT-L-14':
            variant = 'ViT-L-14'
            pretrained = 'datacomp_xl_s13b_b90k'
        model, _, image_preprocess = open_clip.create_model_and_transforms(model_name=variant, pretrained=pretrained, device=device)
        model = model.eval()
        clip_model = CLIPWrapper(model, device) 
        return clip_model, image_preprocess

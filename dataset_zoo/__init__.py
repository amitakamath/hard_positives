from .aro_datasets import VG_Relation, VG_Attribution, COCO_Order, Flickr30k_Order, VL_Checklist_Relation, VL_Checklist_Attribute, VL_Checklist_Object, CREPE_COCO_Negate
...
def get_dataset(dataset_name, image_preprocess=None, text_perturb_fn=None, image_perturb_fn=None, download=False, *args, **kwargs):
    ...
    elif dataset_name == 'VL_Checklist_Relation':
        return get_vl_checklist_relation(image_preprocess=image_preprocess, text_perturb_fn=text_perturb_fn, image_perturb_fn=image_perturb_fn, download=download, *args, **kwargs)
    elif dataset_name == 'VL_Checklist_Attribute':
        return get_vl_checklist_attribute(image_preprocess=image_preprocess, text_perturb_fn=text_perturb_fn, image_perturb_fn=image_perturb_fn, download=download, *args, **kwargs)
    elif dataset_name == 'VL_Checklist_Object':
        return get_vl_checklist_object(image_preprocess=image_preprocess, text_perturb_fn=text_perturb_fn, image_perturb_fn=image_perturb_fn, download=download, *args, **kwargs)
    elif dataset_name == 'CREPE_COCO_Negate':
        return get_crepe_coco_negate(image_preprocess=image_preprocess, text_perturb_fn=text_perturb_fn, image_perturb_fn=image_perturb_fn, download=download, *args, **kwargs)
    else:
